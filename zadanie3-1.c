...
void bernoulli_gen(int v[], int n, double probability) {
	for (int i=0; i<n; i++) {
		int a = rand_from_interval(0, RAND_MAX);
		if (a<probability*RAND_MAX)
			v[i] = 1;
		else
			v[i] = 0;
	}
...
void monty_hall(int n, int *p_switch_wins) {
	for (int i=0; i<n; i++) {
		int win_door = rand_from_interval(1, 3);
		int first_ch = rand_from_interval(1, 3);
		if (first_ch!=win_door)
			*p_switch_wins += 1;
	}
}
